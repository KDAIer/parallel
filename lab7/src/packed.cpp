...
int main()
{
    ...
    for (ln2 = 1; ln2 <= 20; ln2++)
    {
        ...
        for (icase = 0; icase < 2; icase++) // 第一次 icase=0：验证精度; 第二次 icase=1：计时性能测试
        {
            ...
            // 每个进程 pack local_x 到 sendbuf
            int pack_size;
            MPI_Pack_size(2 * local_n, MPI_DOUBLE, MPI_COMM_WORLD, &pack_size);
            vector<double> sendbuf(pack_size);
            int pos = 0; // 位置指针
            MPI_Pack(local_x, 2 * local_n, MPI_DOUBLE, sendbuf.data(), pack_size, &pos, MPI_COMM_WORLD);

            // 跟进程准备recvbuf, 大小为 pack_size * size
            vector<char> recvbuf;
            if (rank == 0)
            {
                recvbuf.resize(pack_size * size);
            }

            MPI_Gather(sendbuf.data(), pack_size, MPI_PACKED, recvbuf.data(), pack_size, MPI_PACKED, 0, MPI_COMM_WORLD);
            // 进程0 unpack recvbuf 到 x
            if (rank == 0)
            {
                for (int p = 0; p < size; ++p)
                {
                    int pos2 = p * pack_size;
                    MPI_Unpack(recvbuf.data(), size * pack_size, &pos2,
                               x + p * 2 * local_n, 2 * local_n, MPI_DOUBLE,
                               MPI_COMM_WORLD);
                }
            }
